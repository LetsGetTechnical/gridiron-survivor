...
BaaS one of the leaders in the market. There are several features that
we can lean heavily on and I have summarized a few that I think will
really make GIS a great experience for the end user.
Auth signin/signup

GIS will have multiple users with varying levels of access. With
Appwrite we are able to configure and manage permissions relatively
...
database and storage also optimizes our use of products. with use of
the SDK’s we are able to quickly implement the auth to whatever level
...
As it's not yet available on cloud we can still set up the email or
...
an sms (if user opts) of weekly results, or that could also be an email.
...
use of the app. Thus making it an overall better experience for the
user.

![Screenshot 2024-04-18 143112](https://github.com/LetsGetTechnical/gridiron-survivor/assets/66279068/33471420-002f-4677-bfd0-d83f06ddcba7)

# Databases storage of users and user data, login info etc

With Appwrite, we have several options of how to use the database
and storage features. The option of server side storage is available
using the server SDK. I am not sure the full extent of the data we will
...
Realtime is one of the most important features to the build. Through
the use of the subscribe method, we are able to listen to events and
...
currently a real time option for server side SDK’s.
...
For someone starting out, it's a very turnkey solution that gives us full
