...
- Since we are using Next.js to build our app, we should use server components where
  ...
  Solution:
  I recommend we use Shadcn. It has many benefits which will behoove not only our
  ...
  customizability in mind. Essentially, we only install the components that we actually need - our
  code bundle will have zero bloat from unused components. This, in addition to its use of
  Tailwind (which, at build time, only includes utility classes that are actually being used) makes
  Shadcn an ideal choice for performance reasons.
  ...
  be used Client Components nested within Server Components, and not as Server Components
  ...
  in our own accessibility features to the components.
