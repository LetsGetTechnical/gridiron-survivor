...
repo, aligning with our the philosophy that solutions across the app should be as centralized as
possible. It is also simple to create custom workflows, allowing us to set up our CI/CD pipeline to
our exact needs.

GitHub Actions is a robust CI/CD tool which will enable us to quickly iterate on our code, while
...
- If we have any problems using GitHub Actions, the documentation and community
  around the tool is robust. Solutions should never be far.
...
Once the develop branch is ready to go for the release, we must do the following:

1. We pause any merge into develop
2. Once that’s done, under Github repository releases section, select create a new release option
3. Click Choose a tag button, and create a tag named v.#.#.#-alpha in tag. Should only be done when pushing to production
4. Target should be on main
5. Name the release title as the applications version (v.#.#.#)
...
2. MINOR version changes add functionality in a backwards-compatible manner, and
3. PATCH version changes are made to correct backwards-compatible bugs.
